...
const Icon = ({ userId }) => {
  ...
  const handleLogout = () => {
    // Logout işlemi gerçekleştirildiğinde giriş sayfasına yönlendir
    navigate('/login');
  };
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div ref={dropdownRef} className="absolute z-10 mt-2 w-32 bg-white rounded-md shadow-lg">
          <a href="#" className="block px-4 py-2 text-gray-800 hover:bg-gray-400">Seçenek 1</a>
          <a href="#" className="block px-4 py-2 text-gray-800 hover:bg-gray-400">Seçenek 2</a>
          <a href="#" className="block px-4 py-2 text-gray-800 hover:bg-gray-400">Seçenek 3</a>
          <a href="login" onClick={handleLogout} className="block px-4 py-2 text-gray-800 hover:bg-gray-400">Çıkış Yap</a>
        </div>
      )}
    </div>
  );
